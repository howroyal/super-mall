...
/*
 * the initail const @Author: dingyantao @CreateDate: 2013-8-22
 */
; (function ($, undefined) {
    $.extend({
        /**
        ...
        template: {
            ...
            replace: function (pTemplate, pData) {
                ...
                if (t) {
                    ...
                    return t(pData);
                    /*
					 * } catch (e) { $.error("TRplace Error:data error"); return
					 * ""; }
					 */
                } else {
                    return "";
            ...
            /*
			 * get .js document which isnot UTF-8 encoding,the return data
			 * should be place into
			 */
            ...
            /*
			 * get session storge,if not exist ,get data from the server @param
			 * {string} the hash key name of the data @param {string} the url
			 * where to get data from @param {string} the arguments used in the
			 * request @param {function|null} the callback function @param
			 * {boolen|null} if use jsonp
			 */
            ...
            /*
			 * get local storge,if not exist ,get data from the server @param
			 * {string} the hash key name of the data @param {string} the url
			 * where to get data from @param {string} the arguments used in the
			 * request @param {function|null} the callback function @param
			 * {boolen|null} if use jsonp
    ...
    $.fn.extend({
        ...
        /*
		 * dom mask $(dom).mask() @param {bool} pFlag
		 * ��������false��λ��������м䣬true���ı�����ԭ��λ��
		 */
        mask: function (pFlag) {
	 ...
	 * browser internal
	 * 
	 * @cfg {Array} [IsChrome,IsFirefox,IsOpera,IsSafari,IsIE,IsIE6,IsIpad]
	 *      ���������bool
	 */
    ...
    var _load = new function () {
        ...
        /*
		 * get javascripts from a merge page
		 * $.load.mergeJs("/r1.js,/r2.js,r3.js","merge.js?src="); @param
		 * {string} srcs @param {string} mergePage @param {function|null}
		 * callback function,arguments followed
		 */
        ...
        /*
		 * add one javascript @param {string} src @param {object|null} callback
		 * function,arguments followed @param {boolen} not use releaseNo
		 */
            ...
            var flagsCache = {},
                ...
                Callbacks = function (flags) {
                    $.extend(true, this, {
                        ...
                        stack: [],
                        /*
						 * memory: undefinde, firing: undefinde, firingStart:
						 * undefinde, firingLength: undefinde, firingIndex:
						 * undefinde,
						 */
                        needSort: false
                    });
    ...
    window.$$ = $$;
    /*
	 * var Mining = function () {//����ھ� if($("#header_box").length > 0){ var
	 * _userAgent = window.navigator.userAgent.toLowerCase(); var mousePosition =
	 * function (event) { if (event.pageX) { return { x: event.pageX, y:
	 * event.pageY } } else { return { x: event.clientX +
	 * (document.documentElement.scrollLeft || document.body.scrollLeft), y:
	 * event.clientY + (document.documentElement.scrollTop ||
	 * document.body.scrollTop) } } }, getPos = function (pEvent) { var _pos =
	 * mousePosition(pEvent); var _x = $("#header_box").offset().left; return
	 * "x=" + (_pos.x - _x) + "&" + "y=" + _pos.y + "&"; }, getParentId =
	 * function (pSon) { var _node = pSon; if (_node.id) return _node.id; while
	 * (_node.parentNode != null) { _node = _node.parentNode; if (_node.id) {
	 * return _node.id; } } }, getHref = function (pSon) { var _node = pSon; if
	 * (_node.tagName == "A") { return "h=" + encodeURIComponent(_node.href) +
	 * "&"; } while (_node.parentNode != null) { _node = _node.parentNode; if
	 * (_node.tagName == "A") { return "h=" + encodeURIComponent(_node.href) +
	 * "&"; } } }, getScreen = function () {//�ֱ��� return "s=" +
	 * window.screen.width + "*" + window.screen.height + "&"; }, getParam =
	 * function (pEvent) { return [ getHref(pEvent.target), getPos(pEvent), "n=" ,
	 * _userAgent ,"&", "dt=", pEvent.target.tagName, "&", "t=", pEvent.button,
	 * "&", "i=", getParentId(pEvent.target), "&", getScreen() ].join(''); },
	 * init = function () { if($("#header_box").length > 0){
	 * $(document).bind("mousedown", function (event) {
	 * $.get($("#header_box").data("url") + "/Home/SaveMining?" +
	 * getParam(event)); }); } }, getData = function(){ return { userAgent:
	 * _userAgent, screenWidth: window.screen.width, screenHeight:
	 * window.screen.height } }, saveMining = function(){
	 * if($("#header_box").length > 0){ $.post($("#header_box").data("url") +
	 * "/Home/SaveMining",{ UILogObject:$.jsonToString(getData()) }); } },
	 * pageLode = function(){ if($$.cookie("MINING_PAGELODE") == null){
	 * $$.cookie("MINING_PAGELODE",true,{ expires: 30, path:"/", domain:
	 * ".springtour.com" }); saveMining(); } } //init(); pageLode(); } } new
	 * Mining();
	 */
})(jQuery, undefined);
(function (g) {}(window));